...
class JwtBorer(override val clock: Clock)
    extends JwtCore[JwtHeader, WebappJwtToken]
    with JwtBorerCodec:
  ...
  protected def parseClaim(claim: String): WebappJwtToken =
    Json.decode(claim.getBytes).to[WebappJwtToken].value

  protected def extractAlgorithm(header: JwtHeader): Option[JwtAlgorithm] =
    header.algorithm
  protected def extractExpiration(claim: WebappJwtToken): Option[Long] =
    claim.expirationTime.map(_.getEpochSecond)
  protected def extractNotBefore(claim: WebappJwtToken): Option[Long] =
    claim.notBefore.map(_.getEpochSecond())

  def decodeAllNoSignatureCheck(token: String): Try[(JwtHeader, WebappJwtToken, String)] =
    decodeAll(token, noSigOptions)

  def decodeNoSignatureCheck(token: String): Try[WebappJwtToken] =
