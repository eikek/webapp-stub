...
final class DefaultJwtVerify[F[_]: Async](
  ...
  def tryDecode(issuer: Uri, token: String): EitherT[F, JwtError, WebappJwtToken] =
    EitherT(state.get.flatMap(_.validate(issuer, clock, token)))

  def tryDecodeOnly(token: String): F[Either[JwtError, WebappJwtToken]] =
  ...
  def updateCache(issuer: Uri, token: String)(
      jwtError: JwtError
  ): F[Either[JwtError, WebappJwtToken]] =
    jwtError match
  ...
  def readIssuer(claim: WebappJwtToken): Either[JwtError, Uri] =
    for
      issuerUri <- Uri
  ...
  def fetchJWKSGuarded(issuer: Uri, claim: WebappJwtToken): EitherT[F, JwtError, Jwks] =
    for
  ...
  def fetchJWKS(issuerUri: Uri, claim: WebappJwtToken): EitherT[F, JwtError, Jwks] =
    for
      _ <- EitherT.right(
