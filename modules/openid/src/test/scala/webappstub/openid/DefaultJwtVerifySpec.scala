...
class DefaultJwtVerifySpec
  ...
  val issuer: Uri = uri"http://wasdev:8180/realms/MyRealm"
  val jwtConfig = JwtVerifyConfig.default.copy(allowedIssuerUrls =
    List(UrlPattern.unsafeFromString("http://wasdev:8180"))
  )
  ...
  test("fetch jwks if validation fails, succeeding with new one"):
    val testClientRoutes = HttpRoutes.of[IO] {
      case GET -> Root / "realms" / "MyRealm" / ".well-known" / "openid-configuration" =>
        Ok(configData)

      case GET -> Root / "realms" / "MyRealm" / "protocol" / "openid-connect" / "certs" =>
        Ok(jwks)
    }
  ...
  test("stop on invalid issuer"):
    ...
    val allowedIssuers = Nil // List(UrlPattern.unsafeFromString("*.myserver.com"))
    for
  ...
  test("scope jwks by issuer uri"):
    val issuer2: Uri = uri"http://wasdev:8180/realms/master"
    val testClientRoutes = HttpRoutes.of[IO] {
      case req @ GET -> Root / "realms" / "master" / ".well-known" / "openid-configuration" =>
        Ok(configData2)
      case req @ GET -> Root / "realms" / "MyRealm" / ".well-known" / "openid-configuration" =>
        Ok(configData)
      case req @ GET -> Root / "realms" / "master" / "protocol" / "openid-connect" / "certs" =>
        Ok(jwks2)
      case req @ GET -> Root / "realms" / "MyRealm" / "protocol" / "openid-connect" / "certs" =>
        Ok(jwks)
      case _ => NotFound()
    }
    val expected = Map(
