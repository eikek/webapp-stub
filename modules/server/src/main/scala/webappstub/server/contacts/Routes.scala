...
import webappstub.server.contacts.Model.ContactEditForm
import webappstub.server.contacts.Views.notFoundPage
...
final class Routes[F[_]: Async](api: RoutesApi[F]) extends Htmx4sDsl[F]:
  def routes: HttpRoutes[F] = HttpRoutes.of:
    ...
    case GET -> Root / "contacts" / Params.ContactId(id) =>
      for {
        c <- api.findById(id)
        view = c
    ...
    case GET -> Root / "contacts" / Params.ContactId(id) / "edit" =>
      for {
    ...
    case req @ POST -> Root / "contacts" / Params.ContactId(id) / "edit" =>
      for {
        formInput <- req.as[ContactEditForm]
    ...
    case DELETE -> Root / "contacts" / Params.ContactId(id) =>
      for {
        found <- api.delete(id)
        resp <-
