...
final case class WebappJwtToken(
  ...
  lazy val isAdmin =
    realmAccess.exists(_.roles.contains("webappstub-admin"))

object WebappJwtToken:
  ...
  given Decoder[WebappJwtToken] = MapBasedCodecs.deriveDecoder
  given Encoder[WebappJwtToken] = MapBasedCodecs.deriveEncoder
